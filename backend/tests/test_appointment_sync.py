...
def test_sync_adds_new_appointments_and_reports_counts(client, db, monkeypatch, app):
    ...
    assert body == {"added": 1, "updated": 0, "skipped": 0, "fetched": 1}
...
def test_sync_skips_an_event_already_in_the_database(client, db, monkeypatch, app):
    ...
    assert response.status_code == 200
    assert response.get_json() == {"added": 0, "updated": 0, "skipped": 1, "fetched": 1}
...
def test_sync_skips_an_event_with_no_invitee_email(client, db, monkeypatch, app):
    ...
    assert response.status_code == 200
    assert response.get_json() == {"added": 0, "updated": 0, "skipped": 0, "fetched": 1}
    assert Appointment.query.count() == 0


def test_sync_reconciles_a_cancellation_on_an_existing_appointment(client, db, monkeypatch, app):
    app.config["CALENDLY_API_ENABLED"] = True
    app.config["CALENDLY_ACCESS_TOKEN"] = "fake-token"
    app.config["CALENDLY_ORG_URI"] = "https://api.calendly.com/organizations/fake-org"
    headers, _ = _login(client, db)
    ...
    cancelled_event = _event("evt-1", status="canceled")
    cancelled_event["cancellation"] = {"reason": "Client asked to reschedule."}

    import app.services.appointment_sync_service as sync_service

    monkeypatch.setattr(sync_service, "list_scheduled_events", lambda *a, **k: [cancelled_event])
    monkeypatch.setattr(sync_service, "list_event_invitees", lambda event_uri: [_invitee("inv-1")])

    response = client.post("/api/admin/appointments/sync", headers=headers)

    assert response.status_code == 200
    assert response.get_json() == {"added": 0, "updated": 1, "skipped": 0, "fetched": 1}

    db.session.refresh(existing)
    assert existing.status == "cancelled"
    assert existing.cancel_reason == "Client asked to reschedule."

    actions = {log.action for log in AuditLog.query.filter_by(entity_type="appointment").all()}
    assert "sync_update" in actions


def test_sync_never_reverts_a_completed_appointment_even_if_calendly_shows_it_cancelled(client, db, monkeypatch, app):
    app.config["CALENDLY_API_ENABLED"] = True
    app.config["CALENDLY_ACCESS_TOKEN"] = "fake-token"
    app.config["CALENDLY_ORG_URI"] = "https://api.calendly.com/organizations/fake-org"
    headers, _ = _login(client, db)
    ...
    monkeypatch.setattr(sync_service, "list_scheduled_events", lambda *a, **k: [_event("evt-1", status="canceled")])
    monkeypatch.setattr(sync_service, "list_event_invitees", lambda event_uri: [_invitee("inv-1")])
...
def test_editor_can_trigger_sync_but_only_admin_can_delete(client, db, monkeypatch, app):
    ...
    assert response.status_code == 200
    assert response.get_json() == {"added": 0, "updated": 0, "skipped": 0, "fetched": 0}
