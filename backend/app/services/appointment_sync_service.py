...
def sync_appointments_from_calendly(admin_id, request):
    """Returns {"added": int, "skipped": int, "fetched": int}. Raises
    CalendlyApiError if Calendly itself couldn't be reached/queried - the
    route translates that into a clean error response instead of a raw 500.

    Duplicate handling is intentionally simple: any event whose
    calendly_event_id already has a row (created here in a previous sync,
    or via the embed) is counted as skipped and left untouched, rather than
    overwritten - an admin may have already edited its status/notes, and
    this action's job is to add what's missing, not to reconcile every
    field on every run.
    """
    now = utcnow()
    ...
    for event in events:
        ...
        exists = db.session.query(
            Appointment.query.filter_by(calendly_event_id=event_id).exists()
        ).scalar()
        if exists:
            skipped += 1
            continue
    ...
    record_audit_log(
        ...
        action="sync",
        entity_type="appointment",
        details={"added": added, "skipped": skipped, "fetched": len(events)},
        request=request,
    )
    db.session.commit()

    logger.info("Calendly sync by admin %s: %d added, %d skipped, %d fetched", admin_id, added, skipped, len(events))
    return {"added": added, "skipped": skipped, "fetched": len(events)}
