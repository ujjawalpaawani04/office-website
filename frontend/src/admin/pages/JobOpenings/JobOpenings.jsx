...
export default function JobOpenings() {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await jobOpeningsApi.remove(pendingDelete.id);
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        actions={(row) => (
          <div className="flex items-center justify-end gap-1">
            ...
            <button type="button" onClick={() => setPendingDelete(row)} aria-label={`Close ${row.title}`} className="rounded-lg p-2 text-secondary/60 hover:bg-red-50 hover:text-red-600">
      ...
      <ConfirmDialog
        ...
        title={`Close "${pendingDelete?.title}"?`}
        description="It will be hidden from the public Career page. Existing applications are kept."
